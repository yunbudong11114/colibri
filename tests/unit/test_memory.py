...
def test_context_loads_memory_and_user_files(tmp_path):
    ...
    assert result.files == ["SOUL.md", "USER.md", "MEMORY.md"]
    assert "Always-on memory:" in result.text
    assert result.text.index("[SOUL.md]") < result.text.index("[USER.md]") < result.text.index("[MEMORY.md]")
    assert "Warm, practical, and concise" in result.text
...
def test_context_bootstraps_sample_files_when_memory_root_is_missing(tmp_path):
    ...
    assert "type: soul" in (root / "SOUL.md").read_text(encoding="utf-8")
    ...
    assert "首次真实写入时直接覆盖样例" in soul_text
...
def test_context_obeys_character_budget(tmp_path):
    root = tmp_path / "memory"
    root.mkdir(parents=True)
    (root / "SOUL.md").write_text("S" * 80, encoding="utf-8")
